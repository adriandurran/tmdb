...
class AdminEditCompetency extends Component {
  ...
  updateCompetency(values) {
    ...
    adminUpdateComp(comp._id, upComp).then((res) => {
      ...
      if (res.status === 200) {
        ...
        message.content = `Something went wrong updating this Competency. Error: ${res.data}`;
  ...
  deleteCompetency = (e) => {
    const { adminDeleteCompetency, comp, history } = this.props;
    adminDeleteCompetency(comp._id).then((res) => {
      if (res) {
        console.log('competency deleted');
        history.push('/admin/competency-manager');
      }
    });
  };
